...
public final class ToolDao{
  ...
  public void queryList(final RoutingContext context,final String sql,final List<Object> params,final ArrayList<String> columns){
  ...
  public void queryMap(final RoutingContext context,final String sql,final List<Object> params,final ArrayList<String> columns){
    client.getConnection((result) ->{
      if(result.succeeded()){
        final SqlConnection conn = result.result();
        conn.preparedQuery(sql).execute(Tuple.wrap(params),rows ->{
